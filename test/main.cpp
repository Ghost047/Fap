...
//  IntegerType ci1((uint64_t) i1, 60, true), ci2((uint64_t) i2, 60, true), ci3(
//      (uint64_t) i3, 60, true);
//  cout << "Add : " << i1 << "\n" << i2 << "\n" << i1 + i2 + i3 << "\n";
//  cout << ci1 << "\n" << ci2 << "\n" << ci1 + ci2 + ci3 << "\n";
//  cout << (int) ci1 << "\n" << (int) ci2 << "\n" << ci3 << "\n"
...
//  }
  ...
  if (alg == "test") {
    ...
    while (1) {
      ...
      ::fap::FloatingPointType::test(-GENERATE_RAND_FLOAT, GENERATE_RAND_FLOAT);
      ::fap::FloatingPointType::test(-GENERATE_RAND_FLOAT, -GENERATE_RAND_FLOAT);
      ::fap::FloatingPointType::test(GENERATE_RAND_DOUBLE, GENERATE_RAND_DOUBLE);
      ::fap::FloatingPointType::test(GENERATE_RAND_DOUBLE, -GENERATE_RAND_DOUBLE);
      ::fap::FloatingPointType::test(-GENERATE_RAND_DOUBLE, GENERATE_RAND_DOUBLE);
      ::fap::FloatingPointType::test(-GENERATE_RAND_DOUBLE, -GENERATE_RAND_DOUBLE);
    }
  ...
  if (alg == "test_vivado") {
    ...
    while (1) {
      ...
      ::fap::vivado::FloatingPointTypeVivado<11, 52>::test(-GENERATE_RAND_FLOAT,
                                            -GENERATE_RAND_FLOAT);
      ::fap::vivado::FloatingPointTypeVivado<11, 52>::test(GENERATE_RAND_DOUBLE,
